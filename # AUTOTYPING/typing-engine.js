...
function pauseOrResumeTyping(state) {
    ...
    const resumeCooldownMs = Number.isFinite(state.resumeCooldownMs) ? Math.max(0, state.resumeCooldownMs) : DEFAULT_RESUME_COOLDOWN_MS;
...
async function typeLineUnit(unit, state, sleepFn, getNow, typeChar, typeChunk) {
    ...
    const startIndex = Number.isInteger(state.lineCharIndex) ? Math.max(0, state.lineCharIndex) : 0;
