...
export class CustomizeView extends LitElement {
    ...
    getSidebarSections() {
        return [
            ...
            { id: 'aiProvider', name: 'AI Provider', icon: 'ai' },
    ...
    connectedCallback() {
        ...
        // Listen for external AI provider changes (e.g. shortcut toggle)
        if (window.require) {
    ...
    getKeybindActions() {
        return [
            ...
            // },
            {
                ...
                description: 'Toggle between Gemini and Groq (Ctrl+Alt+Enter)',
                fixed: true,
            },
        ];
    }
    ...
    async handleAiProviderChange(e) {
        ...
        await cheatingDaddy.storage.updatePreference('aiProvider', this.aiProvider);

        // Notify other components (like the header badge)
        if (window.require) {
            const { ipcRenderer } = window.require('electron');
            ipcRenderer.send('ai-provider-changed-notify', this.aiProvider);
        }
        ...
        return html`
            ...
            <div class="form-grid">
                <div class="form-group">
                    <label class="form-label">
                        Provider
                        <span class="current-selection">${this.aiProvider === 'groq' ? 'Groq (Llama)' : 'Google Gemini'}</span>
                    </label>
