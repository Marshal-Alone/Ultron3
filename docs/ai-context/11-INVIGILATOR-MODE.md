...
### 2. The Keyboard Injector (`src/bin/AutoTyper/Program.cs` / `AutoTyper.exe`)
...
- **Stealth / Focus Safety**:
  - The script explicitly calls `GetAsyncKeyState` to check if the user is physically holding `Ctrl`, `Alt`, or `Win`. If they are, it pauses typing. This guarantees the AutoTyper won't accidentally trigger a browser shortcut (like `Ctrl + Enter`) which could close the exam.
  - Characters that require `Ctrl` or `Alt` combinations (as determined by `VkKeyScan`) are injected using the `KEYEVENTF_UNICODE` flag instead of standard Virtual Keys. This bypasses the need to press modifiers altogether, remaining undetectable to keyloggers looking for unnatural modifier usage.
